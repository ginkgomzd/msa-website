...
//TODO first we need to iterate imports that are approved for today
...
class EmailPreview {
	...
	public function getClientMailsSent() {
		return $this->client_mails_sent;
	}
		...
		if ( $user_id !== '' && $fetching_date !== '' && $client_id !== '' ) {
			try {
				...
				foreach ( $importids as $importid ) {
					$sql = "INSERT INTO import_daily_mails (import_id,user_id,client_id) VALUES(%d,%d,%d) ON DUPLICATE KEY UPDATE daily_mail_sent = true, daily_mail_sent_time = NOW()";
	...
	function generateUserEmail( $user_id, $fetching_date, $client_id ) {
		...
		//TODO repalce this with new method group_By
...
function updateIndexingCompleted( $import_ids ): bool {
	global $wpdb;
	$listimports = [];
	foreach ( $import_ids as $import ) {
		array_push( $listimports, $import['id'] );
	}
	if ( count( $listimports ) > 0 ) {
		$imports = implode( ',', $listimports );
		$wpdb->query( "UPDATE import_table SET indexing_complete = TRUE WHERE id IN ({$imports})" );
...
// TODO create seperate cron for indexing of main core !
$imports = $wpdb->get_results( "SELECT * FROM import_table WHERE fetching_date = CURDATE() AND indexing_complete = FALSE AND curation_date IS NOT NULL ORDER BY client_id;", ARRAY_A );


if ( empty( $imports ) ) {
	echo 'No Bills To Process. Quitting...';
	exit();
}
$grouped_imports = _group_by( $imports, 'client_id' );

class CronReporter {
	private $wpdb;

	public $indexing_counter = 0;

	public $failed_indexing_counter = 0;

	public $indexed_clients = [];

	private $failed_indexed_clients = [];

	public $mails_sent_counter = 0;

	private $process_started;

	private $process_ended;

	public function __construct() {
		global $wpdb;
		$this->wpdb            = $wpdb;
		$this->process_started = date( 'Y-m-d H:m:s' );
	}

	public function getClientName( $client_id ) {
		$client_name = $this->wpdb->get_col( "SELECT client FROM user_clients WHERE id = {$client_id}" );
	...
	public function addIndexedClient( $client ) {
		array_push( $this->indexed_clients, $client );
	}

	public function addFailedIndexedClient( $client ) {
		array_push( $this->failed_indexed_clients, $client );
	}

	public function generateNotificationBody() {
		$this->process_ended = date( 'Y-m-d H:m:s' );
		$body                = "<h2>Indexing process started at: {$this->process_started} and finished at: {$this->process_ended}</h2>";
		$body                .= "<p>Totaly reindexed no. cores: {$this->indexing_counter}</p>";
		$body                .= "<p>Clients that have cores reindex are: </p><ul>";
		foreach ( $this->indexed_clients as $client ) {
			$body .= "<li>{$client}</li>";
		}
		$body .= "</ul>";
		if ( $this->failed_indexing_counter !== 0 ) {
			$body .= "<p>We have failed to reindex cores for following clients (please contact administrator): </p>";
			foreach ( $this->failed_indexed_clients as $client ) {
				$body .= "<li>{$client}</li>";
			}
			$body .= "</ul>";
		}
		$body .= "<p>Total number of mails sent to clients is: {$this->mails_sent_counter}</p>";

		return $body;
	}

	public function generateStaffNotification() {
		$subject      = "Solr Mail Import Notification " . date( 'Y-m-d H:m:s' );
...
foreach ( $grouped_imports as $client => $imports ) {
	$msaBase            = new MSABase();
	$msaBase->client_id = $client;
	$msaBase->checkSolr();
	if ( isset( $msaBase->solr_active ) && $msaBase->solr_active ) {
		echo 'Client: ' . $client . ' is active';
	} else {
		$create_core = apply_filters( 'solr_create_new_core', $client );
		echo 'Client: ' . $client . ' is inactive';
	}

	//we need to reindex now cores for all clients full re-index -clean
	$solr_result = apply_filters( 'solr_reindex_core', $client );
	if ( $solr_result ) {
		$status = updateIndexingCompleted( $imports );
		if ( $status ) {
			$cronreporter->indexing_counter ++;
			$cronreporter->addIndexedClient( $cronreporter->getClientName( $client ) );
		}
	} else {
		$cronreporter->failed_indexing_counter ++;
		$cronreporter->addFailedIndexedClient( $cronreporter->getClientName( $client ) );
	}
	$mail = new  EmailPreview();
	$mail->sendDailyEmailClientUsers( $client, date( 'Y-m-d' ) );
	$cronreporter->mails_sent_counter += $mail->getClientMailsSent();
}
if(count($grouped_imports) > 0){
	// We need to reindex main core
	$solr_result = apply_filters( 'solr_reindex_core', null );
	if ( $solr_result ) {
		echo 'Finished re-indexing main core';
	}
}
$cronreporter->generateStaffNotification();
// TODO solr core needs to be reindexed!
