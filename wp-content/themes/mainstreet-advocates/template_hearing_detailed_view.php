<?php
/* Template Name: Hearing_Detailed_view */
...
if ( ! isset( $_GET['id'] ) ) {
	...
	$row  = $user->getHearingBillDetail( $id );
}
?>
<!--Facebook share-->
<script>
    ...
    <div class="row">
        ...
        <div class="col-md-9">
            <section id="main-info">
                <h3 class="mb-3">
					<?php if ( ! $user->user_is_visitor ) { ?>
						<?php if ( $row->priority === true ) { ?>
                            <i class="star fas fa-star"></i>
						<?php } else { ?>
                            <i class="star far fa-star"></i>
						<?php }
					} ?>
					<?php if ( isset( $row->legislation->state ) && isset( $row->legislation->type ) && isset( $row->legislation->number ) ) {
						echo esc_html( $row->legislation->state ) . ' ' . esc_html( $row->legislation->type ) . ' ' . esc_html( $row->legislation->number );
					} else {
						echo "Not available.";
					} ?>
                </h3>
                ...
                <table>
                    <tr>
                        ...
                        <td>
							<?php echo esc_html( isset( $row->legislation->state ) ?: "Not available." ) ?>
                        </td>
                    </tr>
                    <tr>
                        <td>
        ...
        <div class="col-md-3">
			...
			<?php } ?>
            <div>
				<?php if ( isset( $row->legislation->textUploaded ) && $row->legislation->textUploaded === '1' ) { ?>
                    <a class="button gradient-bg  btn-small mb-4 w-100"
                       href="<?php echo get_site_url(); ?>/detail-bill-text/?id=<?php echo $row->legexternal_id ?>"
    ...
    $(document).ready(function () {
        ...
        function removeNote(id) {
            $.ajax({
                ...
                data: {"action": "remove", "type": "hearing", "bill_id":<?php echo $row->id?>, 'note_id': id},
                dataType: 'JSON',
        ...
        $('.notes').on('click', '#edit_note', function () {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo get_site_url() ?>/wp-content/themes/mainstreet-advocates/notes_api.php',
                data: {"action": "edit", "note_text": new_text_note, "type": "hearing", "id": note_id},
                dataType: 'JSON',
                success: function (response) {
                    if (response.status === true) {
                        $('.edit-note').remove();
        ...
        $('.notes').on('click', '#new_note_add', function () {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo get_site_url() ?>/wp-content/themes/mainstreet-advocates/notes_api.php',
                data: {"action": "add", "note_text": text_notes, "type": "hearing", "id":<?php echo $row->id?>},
                dataType: 'JSON',
                success: function (response) {
                    if (response.status === true) {
        ...
        $('.fa-star').on('click', function () {
            ...
            $.ajax({
                ...
                data: {"id": <?php echo $row->id?>, "type": "hearing", "action": "priority", "status": status},
                dataType: 'JSON',
                success: function (response) {
                    if (response.status === true) {
                        if (status === 'disable') {
